#ifndef MY_STL_HASHTABLE_H_
#define MY_STL_HASHTABLE_H_
...
struct hashtable_node
{
	...
	hashtable_node(const T& value) 
		:next{ nullptr }, value(value)
	{

	}

	hashtable_node(const hashtable_node& rhs)
		:next{ rhs.next }, value{ rhs.value }
	{

	}

	hashtable_node(hashtable_node&& rhs) noexcept
		:next{ rhs.next }, value{ mystl::move(rhs.value) }
	{
		rhs.next = nullptr;
	}
};
...
struct ht_iterator_base: public mystl::iterator<mystl::forward_iterator_tag, T>
{
	...
	using const_iterator			= ht_const_iterator<T, Hash, KeyEqual>;
...
{
	using base					= ht_iterator_base<T, Hash, KeyEqual>;
	using hashtable				= base::hashtable;
	using iterator				= base::iterator;
	using const_iterator		= base::const_iterator;
	using node_ptr				= base::node_ptr;
	using contain_ptr			= base::contain_ptr;
...
struct ht_const_iterator :public ht_iterator_base<T, Hash, KeyEqual>
{
	...
	using iterator				= base::iterator;
	using const_iterator		= base::const_iterator;
	using node_ptr				= base::const_node_ptr;
	using contain_ptr			= base::const_contain_ptr;
...
//1. 从 p = 101 开始。
//2. 计算下一个素数 p = next_prime(p * 1.7)。
//3. 如果 p 小于(2 << 63)，则返回步骤 2；否则，进入步骤 4。
//4. 结束于 p = prev_prime(2 << 63 - 1)，即 2 的 63 次方减一的前一个素数。
//
//这段代码的目的是生成一系列素数，以便在某些算法中用作哈希表的大小。
		...
		try
		{
			for (size_type i = 0; i < ht.bucket_size_; i++)
			{
				...
				if (cur)
				{
					...
					for (auto next = cur->next = 0; next; next = cur->next)
					{
