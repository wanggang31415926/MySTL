...
/*****************************************************************************************/
// search
// 在[first1, last1)中查找[first2, last2)的首次出现点, 如果子序列不存在返回last1
/*****************************************************************************************/
template <typename ForwardIter1, typename ForwardIter2>
ForwardIter1 search(ForwardIter1 first1, ForwardIter1 last1,
	ForwardIter2 first2, ForwardIter2 last2)
{
	//分别计算[first1, last1)与[first2, last2)的长度
...
{
	using reviter1 = reverse_iterator<BidirectionalIter1>;
	using reviter2 = reverse_iterator<BidirectionalIter2>;
...
ForwardIter1 find_end(ForwardIter1 first1, ForwardIter1 last1,
	ForwardIter2 first2, ForwardIter2 last2)
{
	using Category1 = iterator_traits<ForwardIter1>::iterator_category;
	using Category2 = iterator_traits<ForwardIter2>::iterator_category;
...
/*****************************************************************************************/
template <typename InputIter, typename OutputIter, typename UnaryPerdicate>
OutputIter remove_copy_if(InputIter first, InputIter last,
	OutputIter result, UnaryPerdicate unary_pred)
{
	for (; first != last; ++first)
	{
		if (!unary_pred(*first))
...
/*****************************************************************************************/
template <typename ForwardIter1, typename ForwardIter2, typename BinarayPred>
bool is_permutation_aux(ForwardIter1 first1, ForwardIter1 last1,
	ForwardIter2 first2, ForwardIter2 last2, BinarayPred pred)
{
...
template <typename RandomIter, typename T>
void unckecked_linear_insert(RandomIter last, const T& value)
{
	auto next = last;
	--next;
	while (value < *next)
...
void unchecked_insertion_sort(RandomIter first, RandomIter last)
{
	...
	{
		mystl::unckecked_linear_insert(i, *i);
	}
}
...
void insertion_sort(RandomIter first, RandomIter last)
{
	...
	for (auto i = first + 1; i != last; ++i)
	{
		...
		{
			mystl::unckecked_linear_insert(i, value); 
		}
	}
}
